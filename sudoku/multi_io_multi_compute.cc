...
class SudokuServer{
	...
	SudokuServer(muduo::net::EventLoop* loop,
				...
				int threadnum, int ionum);

	void start(){
		server_.setThreadNum(threadio_num_);
		server_.start();
...
SudokuServer::SudokuServer(muduo::net::EventLoop *loop,
						const muduo::net::InetAddress &listenAddr,
						int threadnum, int ionum)
	:loop_(loop), server_(loop, listenAddr, "Sudoko"), threadnum_(threadnum), threadio_num_(ionum),tp(threadnum, "mypool")
{
...
int main(){
	...
	SudokuServer server(&loop, listenAddr, 10, 2);
	server.start();
	loop.loop();
